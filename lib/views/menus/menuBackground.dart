...
class MenuBackground {
  ...
  void render(Canvas canvas, Size newScreenSize) {
    ...
    objects.forEach((element) {
      double x =
          ((horizontalOffset - element.horizontalOffset) * 8 * element.speed) -
              offset;
      if (element.sprite != cyclistSillouhette && element.sprite != tardis) {
        x = screenSize.width - x;
      }
      element.sprite.renderPosition(canvas, Position(x, element.height),
          size: Position(
              element.scale * offset / 1.5, element.scale * offset / 2));
      if (((horizontalOffset - element.horizontalOffset) * 8 * element.speed) *
              -offset *
              2 >
          screenSize.width) {
...
class MovingObject {
  ...
  MovingObject(
      this.sprite, this.speed, this.horizontalOffset, this.height, this.scale);
}
